...
class ProductMigrator:
    ...
    async def _run_migration_all_collections(self, job_id: str, collection_counts: Dict[str, int]):
        ...
        try:
            ...
            logger.info(f"Migration {job_id} completed: {total_migrated} products migrated")
    ...
    async def _migrate_collection(self, job_id: str, collection_name: str, already_migrated: int) -> int:
        ...
        while True:
            ...
            logger.info(
                f"[{collection_name}] Batch processed: {inserted} inserted, "
                f"{duplicates} duplicates, time: {batch_processing_time:.2f}s"
            )
            ...
            logger.info(
                f"Migration progress: {already_migrated + migrated_count} total migrated"
            )
